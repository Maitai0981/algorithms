...
void build(int node, int start, int end) {
    if (start == end) {
        ...
        // Calcula o valor para o nó atual como o máximo entre os filhos
...
int query(int node, int start, int end, int l, int r) {
    if (r < start || end < l) {
        // Intervalo fora da faixa, retorna um valor "menos infinito" para não afetar a consulta
        return LLONG_MIN;
    }
    if (l <= start && end <= r) {
    ...
    // Retorna o máximo entre as duas consultas
...
int32_t main() {
    ...
    while (q--) {
        ...
        if (tipo == 1) {  // Tipo 1: Consulta o máximo no intervalo [l, r]
            int l, r;
            cin >> l >> r;
